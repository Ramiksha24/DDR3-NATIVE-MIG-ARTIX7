...
The design allows **interactive write–read–compare memory testing** using **VIO (Virtual Input/Output)** for real-time control and observation.
...
## ⏱️ Clocking Strategy (Important Design Choice)
...
2. **WAIT_CALIB** – Wait for MIG calibration to complete
3. **WRITE_CMD** – Issue write command
4. **WRITE_DATA** – Send write data
5. **WRITE_WAIT** – Allow write to complete
6. **READ_CMD** – Issue read command
7. **READ_WAIT** – Wait for read data
8. **COMPARE** – Compare read data with written data
9. **DONE** – Signal test completion
...
- `test_done = 1` → Write–read test completed
- `data_match = 1` → Read data matched written data
- `error_count = 0` → No memory errors detected
